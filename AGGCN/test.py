...
from common.ASA import Attention
...
class RGATABSA(nn.Module):
    def __init__(self, args, emb_matrix=None):
        super().__init__()
        in_dim = args.bert_out_dim*3*15/16
        self.args = args
        ...
        self.dropouts = nn.ModuleList(nn.Dropout(p) for p in np.linspace(0.1,0.1,5))
...
class ABSAEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.Layers.append(nn.Linear(in_dim, in_dim / 2))
        self.Layers.append(nn.Linear(in_dim/2, in_dim / 4))
        self.Layers.append(nn.Linear(in_dim / 4, in_dim / 8))
        self.Layers.append(nn.Linear(in_dim / 8, in_dim / 16))
        self.pos_emb = (
    ...
    def forward(self, inputs):
        ...
        #
        output = torch.cat([syn_enc_outputs, sem_enc_outputs, com_syn_outputs], 1)
        layer_outputs = []
        for layer in enumerate(self.mlp):
            output = F.relu(layer(output))
            layer_outputs.append(output)
        layer_outputs_merge = [x_out for x_out in layer_outputs]
        outputs = torch.stack(layer_outputs_merge, -1)
...
class DoubleEncoder(nn.Module):
    def __init__(self, args, embeddings=None, use_dep=False):
        ...
        self.asy = Attention(args.bert_out_dim, score_function='dot_product')
        self.w_weight = nn.Parameter(torch.FloatTensor(2).normal_(0.5, 0.5))
        if use_dep:
    ...
    def inputs_to_att_adj(self, input, src_mask,aspect_embeding):
        attn_tensor = self.attns(input, input, mask=src_mask,aspect_embedding=aspect_embeding)
        adj_ag, _ = torch.max(attn_tensor,1)
        # h_se_pool = torch.div(torch.sum(attn_tensor, dim=1), l.unsqueeze(1).float())
    ...
    def forward(self, adj, inputs, lengths, adj_pos, relation_matrix=None):
        ...
        outputs_syn = self.gc1(bert_out, adj=adj_pos, type="syn")
        outputs_sem = self.gc2(bert_out, adj=adj_pos, att_adj=att_adj, type="sem")
        outputs_com = self.gc3(bert_out, adj=adj_pos, att_adj=att_adj, type="common")
        # outputs_com = self.w_weight[0]*com_syn + self.w_weight[1]*com_sem
...
class GCN(nn.Module):
    def __init__(self, args, in_dim, out_dim, num_layers):
        ...
        # drop out
        # self.attns = nn.ModuleList(([MultiHeadAttention(self.attention_heads, args.bert_out_dim)
        ...
        self.affine1 = nn.Parameter(torch.Tensor(in_dim, in_dim))
        self.affine2 = nn.Parameter(torch.Tensor(in_dim, in_dim))
    ...
    def SemGCN_layer(self, adj, gcn_inputs, denom, l):
        ...
        gcn_inputs = self.gcn_drop(gAxW) if l < self.layers - 1 else gAxW
        return gcn_inputs
    def forward(self, inputs, adj=None, att_adj=None, type=None):
        # gcn
        ...
        if type == 'sem':
            ...
            for i in range(self.layers):
                outputs_sem = self.SemGCN_layer(att_adj, outputs_sem, denom, i)
            return outputs_sem

        elif type == 'syn':
            for i in range(self.layers):
                outputs_syn = self.SynGCN_layer(adj, outputs_syn, denom, i)
            return outputs_syn
        elif type == 'common':

            inter_adj = self.h_weight[0] * att_adj/2 + self.h_weight[1] * adj/2
            # inter_adj = att_adj + adj / 2
            outputs_dep = inputs
            outputs_ag = inputs
            for i in range(self.layers):
                outputs_syn = self.SynGCN_layer(inter_adj, outputs_syn, denom, i)
                # outputs_sem = self.SemGCN_layer(att_adj, outputs_sem, denom, i)
                # A1 = F.softmax(torch.bmm(torch.matmul(outputs_syn, self.affine1), torch.transpose(outputs_sem, 1, 2)), dim=-1)
                # A2 = F.softmax(torch.bmm(torch.matmul(outputs_sem, self.affine2), torch.transpose(outputs_syn, 1, 2)), dim=-1)
                # outputs_syn, outputs_sem = torch.bmm(A1, outputs_sem), torch.bmm(A2, outputs_syn)
            return outputs_syn
            # return outputs_syn
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.context_layer = nn.Linear(d_model*2, d_model)
    ...
    def attention(self, query, key, query_a=None, key_a=None, mask=None, dropout=None):
        ...
        # context_embedded_q = self.context_for_q(query_a)
        ...
        contextualized_query_layer = \
            (1 - lambda_q) * query + lambda_q * query_a

        # context_embedded_k = self.context_for_k(key_a)
        ...
        contextualized_key_layer = \
            (1 - lambda_k) * key + lambda_k * key_a

        attention_scores = torch.matmul(contextualized_query_layer, contextualized_key_layer.transpose(-1, -2))
        # attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        # new_attention_probs = attention_scores

        if mask is not None:
            scores = attention_scores.masked_fill(mask == 0, -1e9)

        attention_probs = F.softmax(scores, dim=-1)

        if dropout is not None:
            new_attention_probs = dropout(attention_probs)
    ...
    def forward(self, query, key, aspect_embedding=None, mask=None):
        ...
        # print(aspect.size())
        context_embeddings = query
        deep_context_hidden = torch.cat([aspect_embedding, context_embeddings], dim=-1)
        deep_context_hidden = self.context_layer(deep_context_hidden)
        deep_context_hidden += context_embeddings
        if aspect_embedding is not None:
            query_a = self.q_aspect(deep_context_hidden).view(nbatches, -1, self.h, self.d_k).transpose(1,2)
            key_a = self.k_aspect(deep_context_hidden).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
