...
class ABSADataLoader(object):
    ...
    def preprocess(self, data, vocab, args):
        ...
        #     elif aspect_n == 14:
        #         ten4 = ten4 +1
        # print(one,two,tri,four,five,six,seven,night,ni,ten,ten1,ten2,ten3,ten4)
        return processed
    ...
    def __getitem__(self, key):
        ...
        src_mask = get_float_tensor(batch[6], batch_size)
        # mask
        mask = get_float_tensor(batch[7], batch_size)
        # length
        length = torch.LongTensor(batch[8])

        text_raw_bert_indices = torch.LongTensor(batch[9])

        bert_sequence = torch.LongTensor(batch[10])

        bert_segments_ids = get_long_tensor(batch[11], batch_size)
        # label
