...
class RGATABSA(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outputs, loss_similar_syn, loss_similar_sem, sem_adj, adj_pos, adj = self.enc(inputs)
        ...
        return logits,loss_com, loss_similar_syn, loss_similar_sem, sem_adj, adj_pos, adj
...
class ABSAEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        out_com, out_syn, out_sem, bert_out, pool_out, sem_adj, adj_pos, adj = h[0], h[1], h[2], h[3], h[4],h[5],h[6],h[7]
        ...
        return outputs, loss_similar_syn, loss_similar_sem, sem_adj, adj_pos, adj
...
class DoubleEncoder(nn.Module):
    ...
    def inputs_to_att_adj(self, input, src_mask, aspect=None):

        attn_tensor = self.attns(input, input, aspect_embedding=aspect, mask=src_mask)
        attn_tensor1 = torch.sum(attn_tensor, dim=1)
        # attn_tensor = select(attn_tensor1, self.args.top_k) * attn_tensor1
        # print(attn_tensor)
        return attn_tensor1
    ...
    def forward(self, adj, inputs, lengths, adj_pos, relation_matrix=None):
        ...
        outputs_com, inter_adj = self.gc3(bert_out, adj_pos, att_adj, type="common")


        return outputs_com, outputs_syn, outputs_sem, bert_out, bert_pool_output, att_adj, adj_pos, adj
