...
parser.add_argument('--theta', default=1.0e-04, type=float)
parser.add_argument('--w_syn', default=0.25, type=float)
parser.add_argument('--w_sem', default=0.25, type=float)
...
def evaluate(model, data_loader):
    ...
    for i, batch in enumerate(data_loader):
        loss, acc, pred, label, _ ,sem_adj,_,_ = model.predict(batch)
        val_loss += loss
        val_acc += acc
        predictions += pred
        labels += label
...
best_path = model_save_dir + "/best_model.pt_new4.21"
...
for epoch in range(1, args.num_epoch + 1):
    ...
    if epoch == 1 or float(val_acc) > max(val_acc_history):
        ...
        file = open("/nfs/project/ync3/1/RGAT-ABSA-master/RGAT-BERT/saved_models/Laptops/train/msd.txt", 'a')
        file.write("acc" +str(val_acc)+ "seed" + str(args.seed) + str(val_f1) + "\n")  # msgÒ²¾ÍÊÇÏÂÃæµÄHello world!
        file.close()
